...
def warn(user: User, chat: Chat, reason: str, message: Message, warner: User = None) -> str:
    ...
    if num_warns >= limit:
        ...
        if soft_warn:  # kick
            ...
            reply = "So, jetzt reicht's! {} Verwarnungen, {} wurde gekickt!".format(limit, mention_html(user.id, user.first_name))

        else:  # ban
            chat.kick_member(user.id)
            reply = "So, das war's- Hausverbot! {} Verwarnungen, {} wurde verbannt!".format(limit, mention_html(user.id, user.first_name))
    ...
    if match:
        ...
        if res:
            update.effective_message.edit_text(
                "Zweite Chance! Verwarnung entfernt vom Admin {}.".format(mention_html(user.id, user.first_name)),
        ...
        if args[0].isdigit():
            if int(args[0]) < 3:
                ...
                return "<b>{}:</b>" \
                       "\n#SET_WARN_LIMIT" \
                       "\n<b>Admin:</b> {}" \
                       "\nHat das Warnlimit auf <code>{}</code> gesetzt".format(html.escape(chat.title),
