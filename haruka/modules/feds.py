...
def new_fed(bot: Bot, update: Update):
	...
	if chat.type != "private":
		update.effective_message.reply_text("Föderationen können nue per PN erstellt werden!")
		return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if sql.search_fed_by_id(fedd) == False:
            message.reply_text("Gib bitte eine gültige Föderations-ID ein.")
            return
